# Dial list — 2026-09-06

816 leads · 428 with a logged outcome · 388 never called

Same order as dialer/TODAY.csv — row numbers match the dialer.

**1. Absolute Clean & Restoration Inc** (618) 402-8225 — ask for Scott & Stacy Ross (Owners) · `CALLBACK`
   due 2026-08-14 · → Email Friday times — HE ASKED
   > **2026-08-11** has a system said his system has been booking fake appointments and shit in his system. doesnt like it at all. wants me to send him an email with a few times to book an appt for friday. absolutecleanpres@gmail.com

**2. CLEAN Restoration** (217) 440-1292 — ask for Jeff Butler (Owner) · `CALLBACK`
   due 2026-08-19 · → Text Melanie 217-640-1523 the website link — SHE ASKED
   > **2026-08-17** called back. current issues with their AI is. people problem - not passing to the correct person. water damage or fire damage spoke to manager said she would pass along my message to owner. said that they were in contact with another company to switch, however Melanie (manager) said she doesnt know if they got everything they wanted from that company text 2176401523. text that number the website link. said they dont want to switch rn. said they would call if they do.

**3. Brilliant Cleaning and Restoration** (620) 210-0424 — ask for Justin (Owner) · `CALLBACK`
   due 2026-08-19 · → Call Justin back, he initiated once already
   > **2026-08-17** guy called me back. said he was eating lunch will give me a call back in a little bit. no answer 8/17

**4. Midwest Restoration Pros LLC** (314) 221-9640 — ask for Donald Sears (Owner) · `CALLBACK`
   due 2026-08-20 · → Call Michelle Thursday morning
   > **2026-08-17** Next monday. Michelle, Don. thursday, morning. call michelle.

**5. Arkansas Restoration** (501) 753-3600 · `CALLBACK`
   due 2026-08-24 · → Call Monday after 8am
   > **2026-08-17** Monday after - 8,

**6. 13:12 Restoration** (479) 977-1312 — ask for Matt (Owner/partner) · `WARM`
   due 2026-08-20 · → Call Matt. Budget objection → lead with Lead Reactivation: no new spend, works money he already paid for. Also find out what happened with Shad
   > **2026-08-05** Matt is owner or business partner. Called. 947 Am Aug 5. Said money is too tight rn. demo with Shad at 8 tn

**7. MRC Restoration of Southeast Missouri** (573) 200-7272 — ask for Mark Curfman (Founder) · `WARM`
   → Cold call → demo #
   > last: spoke to owner (2026-08-10)

**8. St. Louis Cleaning & Restoration** (314) 439-1665 — ask for Kevin (Owner) · `COLD`
   due 2026-10-01 · → DO NOT CALL until October — he named the date himself
   > **2026-08-11** Kevin, said to give him a call back in early october, said he has an answering service, so i pitched him the lead reactivation and conversion, said right now is his busy season he cant take many more clients. so i said ai receptionist and how answering services are out. he said he cant book an appt rn and said to call back start of october.

**9. Tri Cat Restoration** (417) 200-2018 — ask for Jeff Glass (Owner) UNVERIFIED · `GATEKEEPER`
   → Call back, ask Marlena for the owner by name
   > **2026-08-17** spoke to marlena office manager.

**10. Lakeshore Restoration LLC** (920) 221-1440 — ask for Regal · `CALLBACK`
   due 2026-08-20 · → SEND THE INFO — he asked for it 9 days ago and it was never sent
   > **2026-08-11** office@lakeshorerestorationllc.co Regal. said to send info over. just asked for info.

**11. Flood Out Restoration** (903) 235-2969 — ask for "Wes" (Owner) · `WARM`
   due 2026-08-19 · → Follow up: did the owner call the demo #? Best live thread in the pipeline
   > **2026-08-17** spoke to manager, Tyler, asked for the demo number, said if he liked it hell give me a call back about it. said hes gonna have the owner call the demo number and see if he likes it.

**12. Grethey Rose Construction & Restoration** (309) 648-5540 — ask for Victor Grethey (spoke to him) · `WARM`
   → Email John at jon@gretheyrose.com, then call (309) 648-5540
   > **2026-08-17** John Rose. 3096485540 this is the IT business partner Victor is who i spoke to John Rose. 3096485540 this is the IT business partner Victor is who i spoke to. send jon@gretheyrose.com

**13. Chief Restoration Services** (417) 337-3770 — ask for Christy & Matthew Brown · `GATEKEEPER`
   → Cold call → demo #
   > **2026-08-08** 8-4 on the week Matt

**14. A & D Fire/Water Restoration LLC** (573) 979-0958 · `GATEKEEPER`
   → Verify phone first
   > **2026-08-05** Call went to voicemail.

**15. Burlington Restoration Solutions** (319) 671-6640 — ask for Brandon (PM) · `WARM`
   due 2026-08-20 · → Call Brandon — did the 7:30 Zoom happen? Angle is the AI covering the shifts he can't hire for
   > **2026-08-08** SCHEDULED ZOOM LINK AT 730! PM said his main issue is not having enough employees. noone wants to work anymore. also talked to him about phone calls. Brandon

**16. All-Clean USA** (870) 972-1922 — ask for Brett Overman (Owner) · `GATEKEEPER`
   → Verify phone first
   > **2026-08-05** Got to receptionist. Left message

**17. Frontier Restoration** (913) 800-4980 — ask for Doug Day & Greg Marsden (Owners) · `GATEKEEPER`
   → Cold call, ask for Doug or Greg by name
   > **2026-08-10** Jamie is manager

**18. Emergency Mitigation Services** (913) 773-6777 — ask for Fred Dohogne (Owner & GM) · `GATEKEEPER`
   → Cold call, ask for Fred by name
   > last: gatekeeper (2026-08-10)

**19. First Response Restoration** (913) 839-2051 — ask for David Shoemaker (Owner) · `GATEKEEPER`
   → Cold call → demo #
   > **2026-08-10** David is the manager

**20. (936) 235-1735** (936) 235-1735 · `COLD`
   → Not in pipeline.md

**21. (888) 974-9202** (888) 974-9202 · `COLD`
   → Not in pipeline.md

**22. (417) 604-1859** (417) 604-1859 · `COLD`
   → Not in pipeline.md

**23. (844) 242-9889** (844) 242-9889 · `COLD`
   → Not in pipeline.md

**24. (877) 921-5362** (877) 921-5362 · `COLD`
   → Not in pipeline.md

**25. (614) 412-3994** (614) 412-3994 · `COLD`
   → Not in pipeline.md

**26. Certified Water & Mold Restoration** (816) 835-4959 — ask for Mike Martin (Owner) · `GATEKEEPER`
   → Cold call, ask for Mike by name
   > last: gatekeeper (2026-08-08)

**27. FloodSERV, Inc.** (918) 429-1911 — ask for Damon & Kristi Thompson (Owners) · `GATEKEEPER`
   → Cold call → demo #
   > **2026-08-10** Usually in the morning Damon

**28. Environmental Resources, Inc.** (618) 471-9119 — ask for David & Sherri Henson (Owners) · `GATEKEEPER`
   → Cold call → demo #
   > **2026-08-10** asked for a call back

**29. Pride Cleaning & Restoration** (314) 668-1223 — ask for Jim Seubert (Owner) · `GATEKEEPER`
   → Cold call, ask for Jim by name
   > **2026-08-11** Jim Seubert. left message with receptionist

**30. Red Carpet Flood Restoration of NE Kansas** (785) 766-0439 — ask for Keaton · `GATEKEEPER`
   → Cold call (probe live-answer on 24/7 line)
   > **2026-08-11** asked for a call back left message with receptionist

**31. Neville & Sons** (573) 442-8280 — ask for Kevin & Tony Neville (Owners) · `GATEKEEPER`
   → Verify 24/7 emergency line fit, then call
   > **2026-08-11** Kevin, Tony. First week of September

**32. Menold Construction & Restoration** (309) 263-8728 — ask for Brian (Manager) · `GATEKEEPER`
   → Verify which company this actually is before dialing again
   > **2026-08-17** Brian is the manager. this is bluesky restoration contractors.

**33. Healthy Homes LLC** (309) 418-9511 — ask for Caleb Wilson (Owner) · `GATEKEEPER`
   → Call in the morning, ask for Caleb
   > **2026-08-17** Danielle is the receptionist Caleb, Ganon mornings Danielle is the receptionist Caleb, Ganon mornings. left message for caleb the owner.

**34. Peerless Cleaning & Restoration** (217) 423-7704 — ask for Brad Wike (Owner) · `GATEKEEPER`
   → Cold call, ask for Brad by name
   > last: gatekeeper; 2026-08-24 enriched, owner confirmed as Brad Wike via company's own About page (purchased Peerless from his parents in 2004) — no email found on site or contact form, do not guess (2026-08-17)

**35. J.J. Swartz Co** (217) 877-2611 — ask for Brad Swartz (President) · `GATEKEEPER`
   → Call after 6pm — the receptionist has blocked this twice
   > **2026-08-17** got a receptionist

**36. All Clean Restoration Services Inc** (618) 235-3202 — ask for Mike Nagy (President/Owner) · `GATEKEEPER`
   → Call after 9am, ask for Mike by name
   > **2026-08-17** Got to receptionist. Mike. got to receptionist delores. usually anytime after 9am. Mike.

**37. 4-D Home Solutions** (417) 531-0635 — ask for Anthony Decker (Owner) · `GATEKEEPER`
   → Call Anthony, open with "I called at 10 and didn't get an answer"
   > **2026-08-17** Anthony. Called at 953. He didn’t answer. Didn’t leave a voicemail. Call back and say I called at 10 am and didn’t get an answer. 8/17 - no answer

**38. Madison Property Restoration** (608) 467-3360 — ask for Randy Schmidt (President) · `GATEKEEPER`
   → Call after 6pm, ask for Randy by name
   > **2026-08-17** spoke to gatekeeper again

**39. K-tech Kleening** (800) 215-8324 — ask for Craig (Owner) — confirmed on the call · `GATEKEEPER`
   → Call around 8am, ask for Craig
   > **2026-08-17** Craig is owner, clarence is retired. call around 8 am

**40. American Patriot Restoration** (217) 342-3206 — ask for Curt (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-05)

**41. Vortex Restoration** (316) 541-4362 — ask for Matt & Natalie Inbody · `RETRY`
   → Cold call → demo #
   > **2026-08-05** No answer.

**42. Fast Restoration** (636) 681-3200 — ask for Terry (Owner) · `RETRY`
   → Cold call → demo #
   > **2026-08-05** No answer

**43. Burggraf Disaster** (918) 584-3737 · `RETRY`
   → Cold call (check size)
   > last: no answer (2026-08-08)

**44. Atlas Restoration Specialists** (314) 822-0090 — ask for Brian Mullins (President) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-08)

**45. Ben Davis Restoration** (417) 860-9767 — ask for Shad Friend (Founder) · `RETRY`
   → Cold call → demo #
   > **2026-08-05** No answer. Called at 1030.

**46. Brown's Floor Care & Restoration** (319) 337-7721 · `RETRY`
   → Verify phone first
   > last: no answer (2026-08-08)

**47. Allied Restoration Service** (712) 204-0868 · `RETRY`
   → Verify phone first
   > **2026-08-05** no answer

**48. Chavez Restoration & Cleaning** (785) 232-3779 — ask for Dan & Amanda Chavez · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-08)

**49. Bales Cleaning & Restoration** (636) 498-1000 — ask for Larry Martin (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-08)

**50. Cleaning Restoration Services** (918) 710-0609 — ask for Family-run · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-09)

**51. Clean Green Restoration** (417) 300-5062 — ask for Miles & Tarah McNeill · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-09)

**52. Dalton Brothers Restoration** (816) 616-7667 — ask for Mike Dalton (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-09)

**53. Clinton Water Damage Pro** (563) 503-6066 · `RETRY`
   → Verify phone first
   > last: no answer (2026-08-09)

**54. DryHero Water & Mold** (402) 438-2379 — ask for Ritch Paprocki (Founder) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-09)

**55. Dri Right** (580) 574-2383 — ask for Clayton Hutson (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-09)

**56. Disaster Masters, LLC** (501) 441-4087 — ask for Jake Yancey (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-09)

**57. Kwik Dry LLC** (417) 725-6978 — ask for Family-run · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-10)

**58. Kelley Construction Contractors** (309) 346-6604 — ask for Michael Kelley (President) · `COLD`
   → Cold call → demo #
   > last: no answer (2026-08-10)

**59. Freeman General Contracting** (217) 442-2281 — ask for Jacob & James Freeman · `COLD`
   → Cold call → demo #
   > last: no answer (2026-08-10)

**60. Kramer Carpet Cleaning** (563) 590-5722 — ask for Rob & Russ Kramer · `COLD`
   → Cold call → demo #
   > last: no answer (2026-08-10)

**61. Home Pro Services** (319) 382-0217 · `RETRY`
   → Verify phone first
   > last: no answer (2026-08-10)

**62. Illinois Home Solutions** (309) 300-2112 · `RETRY`
   → Verify phone first
   > last: no answer (2026-08-10)

**63. Extremely Clean** (402) 932-3257 · `RETRY`
   → Verify phone + which city first
   > last: no answer (2026-08-10)

**64. MCC Cleaning & Restoration** (314) 443-5044 — ask for Jessica Vieu (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-10)

**65. Smoke Services Restoration** (618) 234-9696 — ask for Metro East · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-11)

**66. Restoration Connection Inc** (618) 925-0085 — ask for John Fowler (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-11)

**67. Advanced Restorations** (417) 962-8844 · `RETRY`
   → Find number first
   > last: no answer (2026-08-11)

**68. Werner Restoration Services Inc** (563) 322-0535 · `RETRY`
   → Verify phone first
   > last: no answer (2026-08-11)

**69. Mustang Disaster CleanUp** (515) 620-3772 · `RETRY`
   → Verify phone first
   > last: no answer (2026-08-11)

**70. Red River Water Restoration** (405) 565-9833 — ask for Andrew Jones & Les Moore (co-founders) · `RETRY`
   → Cold call → demo #
   > **2026-08-11** spoke to answering service

**71. Brownies Carpet & Upholstery Cleaning & Water Restoration** (920) 788-6852 — ask for Tom & Lisa Brown (Owners) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-11)

**72. Vital Restoration** (217) 679-0254 — ask for Timothy Brown (Owner) · `RETRY`
   → Cold call → demo #
   > last: no answer (2026-08-17)

**73. Tri County Cleaning Systems** (309) 266-5755 — ask for Rob Fountain (Owner) · `RETRY`
   → Cold call, ask for Rob by name
   > last: no answer (2026-08-17)

**74. Restore & More** (913) 247-4379 — ask for Mike (Owner) · `RETRY`
   → Cold call, ask for Mike
   > **2026-08-17** called around 1130 monday

**75. Mr. Power Clean** (417) 881-8989 · `COLD`
   → Find number first

**76. Water Damage 24** (501) 408-4062 · `COLD`
   → Find number first

**77. Patriot Water Damage** (918) 800-3372 · `COLD`
   → Find number first

**78. Phoenix Renovation & Restoration** (913) 599-0055 — ask for Brad Heise (CEO) · `COLD`
   → Cold call (confirm headcount fits ICP)

**79. Repairs Unlimited (RUI Restoration)** (913) 262-6937 — ask for Chris Santiago (CEO) · `COLD`
   → Cold call → demo #

**80. B.J.'s Restoration** (309) 829-1500 · `COLD`
   → Find number first

**81. White Construction Co** (217) 442-7594 · `COLD`
   → Verify phone first

**82. Water Damage Pro of Edwardsville** (618) 800-3633 · `COLD`
   → Verify independence (possible lead-gen network)

**83. Simpson Restoration LLC** (417) 759-5117 · `COLD`
   → Find number first

**84. Bryant Restoration** (573) 366-6351 · `COLD`
   → Verify phone first

**85. Greene Planet** (417) 415-4357 · `COLD`
   → Find number first

**86. Beadles Property Maintenance** (417) 840-3330 · `COLD`
   → Find number first

**87. McDowell Roofing & Restoration** (417) 540-3947 · `COLD`
   → Find number first

**88. True North Exteriors** (417) 437-2707 · `COLD`
   → Find number first

**89. KennCo Construction** (417) 881-8147 · `COLD`
   → Find number first

**90. Compass Restoration LLC** (417) 887-4357 · `COLD`
   → Find number first

**91. RC Restoration LLC** (563) 543-3121 · `COLD`
   → Verify fit (markets as roofing co)

**92. Service Pro Restoration** (515) 517-1283 · `COLD`
   → Verify phone first

**93. Service Pros (Service Restore Pro)** (319) 299-2080 · `COLD`
   → Verify phone first

**94. Michaelis Corporation** (317) 251-1935 · `COLD`
   → Cold call → demo #

**95. Restorex Disaster Restoration** (317) 315-5071 · `COLD`
   → Cold call → demo #

**96. Indiana Restoration & Cleaning Services** (317) 776-9942 · `COLD`
   → Cold call → demo #

**97. J.L. Fox Inc** (317) 255-4144 · `COLD`
   → Cold call → demo #

**98. Absolute Restoration LLC** (317) 871-4074 — ask for Randy George (Owner) · `COLD`
   → Cold call, ask for Randy by name

**99. Icon Property Rescue** (513) 426-6767 · `COLD`
   → Cold call → demo #

**100. Dry Effect Restoration Services** (513) 763-2121 · `COLD`
   → Cold call → demo #

**101. WayMakers Restoration** (614) 325-0678 · `COLD`
   → Cold call → demo #

**102. PHG Restoration Services** (614) 359-3154 — ask for Josh Clifton (Owner) · `COLD`
   → Cold call, ask for Josh by name

**103. AIM Green Restoration** (614) 877-8855 · `COLD`
   → Cold call → demo #

**104. Dry Patrol** (740) 417-9006 — ask for Bryan Daughtry (Founder) · `COLD`
   → Cold call, ask for Bryan by name

**105. Maximum Restoration** (937) 203-0123 · `COLD`
   → Cold call → demo #

**106. Active Restoration** (937) 431-9999 · `COLD`
   → Cold call → demo #

**107. Disaster X** (937) 343-4357 · `COLD`
   → Cold call → demo #

**108. Design Restoration & Reconstruction** (330) 563-0010 · `COLD`
   → Cold call → demo #

**109. Water Extraction Team** (440) 953-0767 · `COLD`
   → Cold call → demo #

**110. Pine Ridge Restoration** (440) 276-3636 · `COLD`
   → Cold call → demo #

**111. Cleveland Masters of Disasters** (216) 663-1111 — ask for Larry · `COLD`
   → Cold call, ask for Larry by name

**112. Integrated Restoration** (216) 361-3473 · `COLD`
   → Cold call → demo #

**113. Disaster Recovery Services** (330) 395-7878 · `COLD`
   → Cold call → demo #

**114. RestoWorks** (330) 574-1514 · `COLD`
   → Cold call → demo #

**115. National Fire & Water Repair** (330) 782-5392 · `COLD`
   → Cold call → demo #

**116. SMB Restoration Specialists** (419) 269-1473 — ask for Jim Mossing (President) · `COLD`
   → Cold call, ask for Jim by name

**117. Service Pros (Fort Wayne)** (260) 253-5820 · `COLD`
   → Cold call → demo #

**118. Water Out** (260) 305-1062 · `COLD`
   → Cold call → demo #

**119. Distinctive Cleaning & Restoration** (260) 250-4664 — ask for Alan & Audrey Falls (Owners) · `COLD`
   → Cold call, ask for Alan or Audrey by name

**120. Hasgoe** (812) 464-2402 · `COLD`
   → Cold call → demo #

**121. Environmental Cleaning Solutions** (812) 602-3839 · `COLD`
   → Cold call → demo #

**122. Water Fire Mold Experts LLC** (812) 431-3533 · `COLD`
   → Cold call → demo #

**123. RAM Restoration** (616) 818-1700 · `COLD`
   → Cold call → demo #

**124. Riverside Restoration** (616) 675-0110 · `COLD`
   → Cold call → demo #

**125. Hadaway Cleaning & Restoration** (269) 375-7522 · `COLD`
   → Cold call → demo #

**126. Bailtek Carpet Cleaning & Water Damage** (269) 484-1011 · `COLD`
   → Cold call → demo #

**127. Dri-Life Restoration** (231) 935-4058 · `COLD`
   → Cold call → demo #

**128. Michigan Flood Pros** (248) 923-5663 · `COLD`
   → Cold call → demo #

**129. weDRY Restoration** (248) 752-3222 · `COLD`
   → Cold call → demo #

**130. M&J Restoration** (734) 472-4414 · `COLD`
   → Cold call → demo #

**131. R & S Restores** (248) 669-2600 · `COLD`
   → Cold call → demo #

**132. Prime Restoration** (586) 277-1069 — ask for Cole Hartwell (Founder/CEO) · `COLD`
   → Cold call, ask for Cole by name

**133. X-Cel Restoration** (248) 414-3200 · `COLD`
   → Cold call → demo #

**134. Anytime Restoration** (248) 965-1079 · `COLD`
   → Cold call → demo #

**135. Action Extraction Inc.** (586) 949-4448 — ask for Charles Bingham (Owner) · `COLD`
   → Cold call, ask for Charles by name

**136. J&L Restoration** (517) 322-3000 · `COLD`
   → Cold call → demo #

**137. Camelot Emergency Water Removal** (734) 766-7856 · `COLD`
   → Cold call → demo # — FLAG: 4 MI branches, may exceed the 15-tech ceiling, confirm size on the call

**138. Apex Restoration & Mitigation** (810) 355-4775 · `COLD`
   → Cold call → demo #

**139. Gohm Insurance Restoration** (989) 752-0606 · `COLD`
   → Cold call → demo #

**140. Exact Recon** (517) 262-3770 · `COLD`
   → Cold call → demo # — outside the named metros, sourced as backup

**141. Doan Restoration of Michigan** (586) 842-8142 · `COLD`
   → Cold call → demo #

**142. The Restoration Doctor** (586) 522-4922 · `COLD`
   → Cold call → demo #

**143. Michigan Fire & Flood Inc** (586) 221-0821 · `COLD`
   → Cold call → demo #

**144. MGM Restoration Inc** (888) 514-4639 · `COLD`
   → Cold call → demo #

**145. 1st Class Carpet Cleaning & Restoration** (248) 491-3088 · `COLD`
   → Cold call → demo #

**146. Emergency Response Services, Inc. (ERSI)** (877) 562-3663 · `COLD`
   → Cold call → demo #

**147. Watermark Restoration Solutions, Inc.** (248) 278-6166 · `COLD`
   → Cold call → demo #

**148. United Restoration LLC** (734) 991-3447 · `COLD`
   → Cold call → demo #

**149. Rendall's Certified Cleaning Services** (810) 225-2184 · `COLD`
   → Cold call → demo #

**150. McCardel Restoration, LLC** (517) 550-5315 · `COLD`
   → Cold call → demo #

**151. The Bel Company** (517) 899-0349 · `COLD`
   → Cold call → demo #

**152. John Grace Restoration & Construction** (616) 438-5315 · `COLD`
   → Cold call → demo #

**153. Revive Restoration, Inc.** (608) 222-9222 · `COLD`
   → Cold call → demo #

**154. Damage Control Inc** (414) 672-3409 · `COLD`
   → Cold call → demo #

**155. Monarch Restoration** (502) 401-1805 · `COLD`
   → Cold call → demo #

**156. Emrick Services** (502) 876-1891 · `COLD`
   → Cold call → demo #

**157. Southern Clean Air Restoration** (502) 270-4449 · `COLD`
   → Cold call → demo #

**158. Nashville Damage Restoration Pros** (615) 436-5759 — ask for Matthew Delaney & Barry Hargrove (Co-Founders) · `COLD`
   → Cold call → demo #

**159. Allegiance Restoration LLC** (901) 245-0039 · `COLD`
   → Cold call → demo #

**160. Landmark Construction & Emergency Services** (901) 452-0390 · `COLD`
   → Cold call → demo #

**161. Mold Masters** (865) 351-6344 · `COLD`
   → Cold call → demo #

**162. Mold Doctors USA** (865) 945-3000 · `COLD`
   → Cold call → demo #

**163. Ethos Restoration** (865) 693-3265 — ask for Jeremy Akers (Founder) · `COLD`
   → Cold call → demo #

**164. Knoxville Restoration Pro** (865) 239-3435 · `COLD`
   → VERIFY FIRST — generic city+service name, the same pattern as the irs-247 network already excluded twice

**165. Spotless** (859) 459-0424 — ask for Paul Craig & Tina (Owners) · `COLD`
   → Cold call → demo #

**166. GAMO Restoration** (859) 267-1446 · `COLD`
   → Cold call → demo #

**167. Reliable Restoration of KY, Inc** (270) 791-3298 — ask for Philip Cunningham · `COLD`
   → Cold call → demo #

**168. Pure Air Solutions, LLC** (270) 774-7472 — ask for Bruce Sullivan (Owner/Operator) · `COLD`
   → Cold call → demo #

**169. Sky Restoration DKI** (270) 253-4354 — ask for Brian Elkins (Owner) · `COLD`
   → Cold call → demo #

**170. Metro Restoration Services, Inc** (502) 637-8767 · `COLD`
   → Cold call → demo #

**171. River City Restoration Group** (502) 966-4044 — ask for David Emrath (President) · `COLD`
   → Cold call → demo #

**172. Bear Restoration** (502) 200-6968 — ask for Emery L. Bear (Owner) · `COLD`
   → Cold call → demo #

**173. DWB Restoration II, Inc** (502) 241-6990 — ask for Jeremy Thomas Stewart (Owner) · `COLD`
   → Cold call → demo #

**174. Eclipse Roofing & Restoration** (502) 266-7510 — ask for James & Melanie Reynolds (Owners) · `COLD`
   → Cold call → demo #

**175. Zipco Restoration** (877) 799-4726 · `COLD`
   → Cold call → demo #

**176. ATP Restoration LLC** (913) 257-5825 · `COLD`
   → Cold call → demo #

**177. Rapid Dry Kansas City** (816) 799-1234 · `COLD`
   → Cold call → demo #

**178. Pure Air LLC** (816) 946-1410 · `COLD`
   → Cold call → demo #

**179. Dog Gone Mold** (816) 547-6816 · `COLD`
   → Cold call → demo #
